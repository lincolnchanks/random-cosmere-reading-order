...
## Known Issues
When you run First Chapter Random or Random Equal Weight, to my knowledge the resulting output.txt file is 1,421 lines long. When you run True Random, however, the file is only 1,418 lines long. This implies that three chapters are missing from cosmere_chapters.json (the file used by True Random). I do not have the time or patience right now to look for those, but in the future I will fix the file if and when I find those chapters.
## Copyright and Info
